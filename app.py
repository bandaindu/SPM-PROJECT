...
Session(app)
mydb=mysql.connector.connect(host='localhost',user='root',password='bindu',db='spm')
app.secret_key=b'\x03H\x8f\xd1"'
@app.route('/')
def welcome():
    ...
    if request.method=='POST':
        ...
        # data={'email':email,'stu_fname':stu_fname,'stu_lname':stu_lname,'password':password}
         ...
         if data==0:
            ...
            body=f'Register otp for SPM application {otp}'
         ...
         #print(stu_fname,stu_lname,email,ph_no,address,password)
         #mysql connection
         '''cursor=mydb.cursor(buffered=True)
...
def allnotes():
    if not session.get('email'):
        ...
        added_by=session.get('email')
        cursor=mydb.cursor(buffered=True) 
        cursor.execute('select nid,title,create_at from notes where added_by=%s',[added_by])
...
def viewall_files():
    if not session.get('email'):
        ...
        added_by=session.get('email')
        cursor=mydb.cursor(buffered=True)
        cursor.execute('select f_id,file_name,create_at from files_data where added_by=%s',[added_by]) 
        data=cursor.fetchall()
    ...
    if session.get('email'):
        ...
        if request.method=='POST':
            ...
            if count==0:
                ...
                body=f"Registation otp for SPM application {url_for('reset',token(data=email),_external=True)}"
                sendmail(to=email,subject=subject,body=body)
                flash('Reset link has been sent to Email')
            else:
    ...
    try:
        ...
        if request.method=='POST':
            ...
            if npassword==cpassword:
                ...
                return 'confirmation password worng'   
...
def getexcel_data():
    if not session.get('email'):
        ...
        cursor=mydb.cursor(buffered=True)
        cursor.execute('select title,note_content,create_at from notes where added_by=%s',[user])
        data=cursor.fetchall()
